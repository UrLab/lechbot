...
class CCC35(BotPlugin):
    ...
    def show_talk(self, talk, url=False, prefix=""):
        ...
        fmt = (
            prefix + "«{title}» [Day{relative_day}, {start_time_dt:%H:%M}], {room_name}"
        )
        ...
        if url:
            fmt_url = (
                " " * len(prefix)
                + "https://fahrplan.events.ccc.de/congress/2018/Fahrplan/events/{id}.html"
            )
            self.say(self.bot.text.blue(fmt_url.format(**talk)), strip_text=False)
    ...
    def get_fahrplan(self, force=False):
        with Persistent("fahrplan.json") as fahrplan:
            if not fahrplan or force:
                x = yield from public_api(
                    "https://fahrplan.events.ccc.de/congress/2018/Fahrplan/events.json"
                )
    ...
    @BotPlugin.command(
        r".*https?://fahrplan\.events\.ccc\.de/congress/2018/Fahrplan/events/(\d+).html"
        + end_url
    )
    ...
    def talks(self, msg):
        ...
        events = sorted(
            ...
            key=lambda x: (
                ...
                dateutil.parser.parse(x["end_time"]),
            ),
        )
        ...
        for event in events:
            if (
                dateutil.parser.parse(event["start_time"])
                < now
                < dateutil.parser.parse(event["end_time"])
            ):
    ...
    def last_update(self, msg):
        ...
        msg.reply(
            "Fahrplan a été mis à jour il y a %i minutes"
    ...
    def room(self, msg):
        ...
        for event in events:
            if (
                dateutil.parser.parse(event["end_time"]) > now
                and event["room"]["name"].lower() == msg.args[0].lower()
            ):
                kept.append(event)
    ...
    def music(self, msg):
        ...
        schedule = yield from public_api(
            "https://fahrplan.events.ccc.de/congress/2018/Lineup/schedule.json"
        )

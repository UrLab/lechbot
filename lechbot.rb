...
lechbot = Cinch::Bot.new do
    ...
    #!open & !close
    close = 0
    on :message, /^\!(open$|close$|open \d+|close \d+|ring)/ do |msg, status|
        if msg.channel == CHANNELS.first
            ...
            begin
                ...
                if status == "open" or status == "close"
                    ...
                    # un temps a été spécifié 
                elsif status =~ /open \d+/ or status =~ /close \d+/ or status == "ring" 

                    begin
                        # récupère le temps
                        time = Integer("#{status}".gsub(/^.* (\d+)/, '\1'))
                        if status != "ring"
                            # donne l’heure de fermeture
                            close = Time.at(Time.now + time * 60).strftime('%H:%M')
                        end
                    rescue Exception => e
                    end

                    if status =~ /open \d+/
                        msg.reply "Le hs restera ouvert #{time} minutes, jusque #{close}"

                    elsif status =~ /close \d+/
                        msg.reply "Le hs fermera dans #{time} minutes, à #{close}"

                    elsif status == "ring"
                        msg.reply "Le hs est ouvert jusque #{close}" 
                ...
                msg.reply "Erreur d'accès à SpaceAPI: #{e} !!! (As-tu attendu 5min depuis le dernier changement de statut ?)"
            end
        end
    end
